...
static void print_usage() {
  std::cerr
      << "Usage: Split long reads into paired-end pseudo-linked reads."
      << PROGNAME
      << " -l L -g G [--fasta -s -d -p P -c C -m M -t T -f FILE] READS \n\n"
         "  -l L        Use L as simulated read length size.\n"
         "  -g G        Use G as Genome size (bp) for calculating tigmint-long "
         "span "
         "parameter as an integer or in scientific notation (e.g. '3e9').\n"
         "  --fasta     Output in fasta format.\n"
         "  -f FILE     Write estimated parameter to FILE. "
         "[tigmint-long.params.tsv]\n"
         "  -s          Calculate span parameter for tigmint-long "
         "automatically.\n"
         "  -c C        Use 'C * sequence coverage' to estimate span "
         "parameter. [0.25]\n"
         "  -d          Calculate dist parameter for tigmint-long "
         "automatically.\n"
         "  -p P        Use P percentile to estimate dist parameter. [50].\n"
         "  -m M        M minimum read length for a read to be considered a "
         "molecule. [2000].\n"
         "  -t T        Use T number of threads (max 6) per input file. [6]\n"
         "  -v          Show verbose output.\n"
         "  --help      Display this help and exit.\n"
         "  --version   Display version and exit.\n"
         "  READS       Space separated list of long reads FASTA/Q files to be "
         "cut."
      << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  bool l_set = false;
  double cov_to_span =
      0.25; // Optimal tigmint-long span is 1/4 sequence coverage
  ...
  static const struct option longopts[] = {
      ...
      {nullptr, 0, nullptr, 0}};
  while ((c = getopt_long(argc, argv, "l:g:o:c:p:sdf:t:", longopts,
                          &optindex)) != -1) {
    switch (c) {
    case 0:
      break;
  ...
  if (t > MAX_THREADS) {
    ...
    std::cerr << (PROGNAME + ' ' + VERSION + ": Using more than " +
                  ...
                  " threads does not scale, reverting to " +
                  std::to_string(MAX_THREADS) + ".\n")
              << std::flush;
  }
  ...
  for (auto &infile : infiles) {
    unsigned flags = 0;
    flags |=
        btllib::SeqReader::Flag::NO_FOLD_CASE; // skip time intensive checking
      ...
      if (remainder != 0) {
        ...
        if (!with_fasta) {
          ...
          if (qual_size == 0) {
            ...
            std::string reverse_qual =
                qual.substr(seq_size - forward_linked_read.size(),
                            forward_linked_read.size());
            std::reverse(reverse_qual.begin(), reverse_qual.end());
            std::cout << reverse_qual << '\n';
          }
        }
  ...
  if (auto_span || auto_dist) {
    ...
    if (auto_dist) {
      if (read_lengths.size() == 0) {
        std::cerr << "long-to-linked-pe: unable to estimate dist parameter due "
                     "to no valid "
                     "lengths"
                  << std::endl;
      } else {
