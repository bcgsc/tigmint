...
#     cut_reads = run_long_to_linked("test_longreads", 500, 2000)
...
#     """
#     cut_reads = run_long_to_linked("test_longreads", 500, 47000)
#     assert len(cut_reads) == 0

# def test_tigmint_molecule():
#     run_tigmint_molecule("test_contig", "test_longreads")

def test_cut_reads():
    if "test_longreads.cut500.fa.gz" in os.listdir():
        with gzip.open("expected_outputs/test_longreads.cut500.fa.gz", "rt") as expected_cuts:
            with gzip.open("test_longreads.cut500.fa.gz", "rt") as outfile:
                output_cuts = outfile.readlines()
                for i, actual in enumerate(expected_cuts):
                    assert output_cuts[i] == actual
        os.remove("test_longreads.cut500.fa.gz")

# def test_tigmint_cut():
#     run_tigmint_cut("test_contig", "test_longreads")